...
import { broadcastCatalogUpdated, broadcastSiteUpdated, CATALOG_CHANNEL, CATALOG_EVENT, SITE_EVENT } from './catalog-revision.js';
...
test('broadcastSiteUpdated uses the same channel with a distinct event', async () => {
  process.env.SUPABASE_URL = 'https://example.supabase.co';
  process.env.SUPABASE_SERVICE_ROLE_KEY = 'service-role-test';
  const originalFetch = globalThis.fetch;
  const calls = [];
  ...
  try {
    const ok = await broadcastSiteUpdated('rev-site');
    assert.equal(ok, true);
    const body = JSON.parse(calls[0].options.body);
    assert.equal(body.messages[0].topic, CATALOG_CHANNEL);
    assert.equal(body.messages[0].event, SITE_EVENT);
    assert.notEqual(SITE_EVENT, CATALOG_EVENT);
  } finally {
    globalThis.fetch = originalFetch;
    delete process.env.SUPABASE_URL;
    delete process.env.SUPABASE_SERVICE_ROLE_KEY;
