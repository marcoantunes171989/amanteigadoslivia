...
export const SQL_USERS = {
  getByEmail: `-- op:get_usuario_email
    SELECT id_usuario_admin, nome_usuario, email_usuario, senha_hash, senha_salt,
           perfil_usuario, ativo, data_ultimo_login
  ...
  insert: `-- op:insert_usuario
    INSERT INTO app.tab_usuario_admin (
      id_usuario_admin, nome_usuario, email_usuario, senha_hash, senha_salt, perfil_usuario, ativo
    ) VALUES ($1, $2, $3, $4, $5, $6, $7)
    RETURNING id_usuario_admin, nome_usuario, email_usuario, perfil_usuario, ativo,
  ...
  countActiveAdmins: `-- op:count_admin_ativos
    ...
    FROM app.tab_usuario_admin
    WHERE perfil_usuario = 'ADMIN' AND ativo = true
  `,
};
...
function normalizePerfil(value, fallback = 'ADMIN') {
...
export async function createUsuario(queryable, dados = {}) {
...
export async function updateUsuario(queryable, id, dados = {}, session = {}) {
  ...
  if (current.perfil_usuario === 'ADMIN' && current.ativo === true && (perfil !== 'ADMIN' || ativo === false)) {
    ...
    const isSelf = session.id_usuario_admin && String(session.id_usuario_admin) === String(id);
    if (total <= 1 && isSelf) {
...
export async function resetUsuarioSenha(queryable, id, senha) {
  if (!id) {
    throw new AdminError(400, 'validation_error', 'Usuário é obrigatório.');
  }
  if (typeof senha !== 'string' || senha.length < 8) {
